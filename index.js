...
const getUsersCreditDetails = arr => {
  ...
  const userCreditDetailsArr = [];
  for (let user of arr) {
    const {firstName, lastName, balance} = user;
    userCreditDetailsArr.push({firstName, lastName, balance})
  }
  return userCreditDetailsArr;
};
...
const genderView = users => {
  // Your code goes here ...
  const filteredByGenderObject = {
    femaleUsers: users.filter(user => user.gender == 'female'),
    maleUsers: users.filter(user => user.gender == 'male')
  }
return filteredByGenderObject;
...
// Bonus - Iteration 6
// ***************************************************************************
const sliceTillComma = (balance) => {
  let commaIndex = balance.indexOf(',');
  return balance.slice(1, commaIndex);
}


const promo20 = users => {
  // Your code goes here ...
  const usersPromo20 = users.filter(user => Number(sliceTillComma(user.balance)) >= 20)
